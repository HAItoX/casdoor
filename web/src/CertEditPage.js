...
class CertEditPage extends React.Component {
  ...
  getCert() {
    CertBackend.getCert(this.state.owner, this.state.certName).then((res) => {
      if (res.data === null) {
    ...
    return (
      <Card
        ...
        title={
          <div>
            {this.state.mode === "add"
              ? i18next.t("cert:New Cert")
              : i18next.t("cert:Edit Cert")}
            &nbsp;&nbsp;&nbsp;&nbsp;
            <Button onClick={() => this.submitCertEdit(false)}>
              {i18next.t("general:Save")}
            </Button>
            <Button
              style={{ marginLeft: "20px" }}
              type="primary"
              onClick={() => this.submitCertEdit(true)}
            >
      ...
      >
        <Row style={{ marginTop: "20px" }}>
          ...
          <Col span={22}>
            <Select
              ...
              onChange={(value) => {
                this.updateCertField("owner", value);
              }}
            >
              {Setting.isAdminUser(this.props.account) ? (
                <Option key={"admin"} value={"admin"}>
                  {i18next.t("provider:admin (Shared)")}
                </Option>
              ) : null}
        ...
        <Row style={{ marginTop: "20px" }}>
          ...
          <Col span={22}>
            <Input
              value={this.state.cert.displayName}
              onChange={(e) => {
        ...
        <Row style={{ marginTop: "20px" }}>
          ...
          <Col span={22}>
            <Select
              virtual={false}
              style={{ width: "100%" }}
              value={this.state.cert.scope}
              onChange={(value) => {
                this.updateCertField("scope", value);
              }}
            >
              {[{ id: "JWT", name: "JWT" }].map((item, index) => (
        ...
        <Row style={{ marginTop: "20px" }}>
          <Col style={{ marginTop: "5px" }} span={Setting.isMobile() ? 22 : 2}>
            {Setting.getLabel(
              ...
              i18next.t("cert:Crypto algorithm - Tooltip")
            )}{" "}
            :
          </Col>
          <Col span={22}>
            <Select
              virtual={false}
              style={{ width: "100%" }}
              value={this.state.cert.cryptoAlgorithm}
              onChange={(value) => {
                ...
                if (value.startsWith("ES")) {
                  ...
                  if (
                    this.state.cert.bitSize !== 1024 &&
                    this.state.cert.bitSize !== 2048 &&
                    this.state.cert.bitSize !== 4096
                  ) {
            ...
            >
              {[
                ...
                {
                  id: "PS256",
                  name: "PS256 (RSASSA-PSS using SHA256 and MGF1 with SHA256)",
                },
                {
                  id: "PS384",
                  name: "PS384 (RSASSA-PSS using SHA384 and MGF1 with SHA384)",
                },
                {
                  id: "PS512",
                  name: "PS512 (RSASSA-PSS using SHA512 and MGF1 with SHA512)",
                },
              ].map((item, index) => (
                <Option key={index} value={item.id}>
                  {item.name}
          ...
          <Row style={{ marginTop: "20px" }}>
            ...
            >
              {Setting.getLabel(
                ...
                i18next.t("cert:Bit size - Tooltip")
              )}{" "}
              :
            </Col>
            <Col span={22}>
              ...
              >
                {Setting.getCryptoAlgorithmOptions(
                  this.state.cert.cryptoAlgorithm
                ).map((item, index) => (
                  <Option key={index} value={item.id}>
                    {item.name}
                  </Option>
        ...
        <Row style={{ marginTop: "20px" }}>
          <Col style={{ marginTop: "5px" }} span={Setting.isMobile() ? 22 : 2}>
            {Setting.getLabel(
              i18next.t("cert:Expire in years"),
              i18next.t("cert:Expire in years - Tooltip")
            )}{" "}
            :
          </Col>
          <Col span={22}>
            <InputNumber
              value={this.state.cert.expireInYears}
              onChange={(value) => {
        ...
        <Row style={{ marginTop: "20px" }}>
          <Col style={{ marginTop: "5px" }} span={Setting.isMobile() ? 22 : 2}>
            {Setting.getLabel(
              ...
              i18next.t("cert:Certificate - Tooltip")
            )}{" "}
            :
          </Col>
          <Col span={editorWidth}>
            <Button
              ...
              disabled={this.state.cert.certificate === ""}
              onClick={() => {
            ...
            <Button
              ...
              disabled={this.state.cert.certificate === ""}
              onClick={() => {
                const blob = new Blob([this.state.cert.certificate], {
            ...
            <TextArea
              ...
              value={this.state.cert.certificate}
              onChange={(e) => {
          ...
          <Col style={{ marginTop: "5px" }} span={Setting.isMobile() ? 22 : 2}>
            {Setting.getLabel(
              ...
              i18next.t("cert:Private key - Tooltip")
            )}{" "}
            :
          </Col>
          <Col span={editorWidth}>
            <Button
              style={{ marginRight: "10px", marginBottom: "10px" }}
              disabled={this.state.cert.privateKey === ""}
              onClick={() => {
            ...
            <Button
              ...
              disabled={this.state.cert.privateKey === ""}
              onClick={() => {
                const blob = new Blob([this.state.cert.privateKey], {
                  type: "text/plain;charset=utf-8",
                });
            ...
            <TextArea
              autoSize={{ minRows: 30, maxRows: 30 }}
              value={this.state.cert.privateKey}
              onChange={(e) => {
            ...
            () => {
              if (exitAfterSave) {
                ...
                this.props.history.push(
                  `/certs/${this.state.cert.owner}/${this.state.cert.name}`
                );
                this.getCert();
              }
            }
    ...
    return (
      <div>
        ...
        <div style={{ marginTop: "20px", marginLeft: "40px" }}>
          <Button size="large" onClick={() => this.submitCertEdit(false)}>
            {i18next.t("general:Save")}
          </Button>
          <Button
            style={{ marginLeft: "20px" }}
