...
class PermissionEditPage extends React.Component {
  ...
  getPermission() {
    PermissionBackend.getPermission(
  ...
  getModels(organizationName) {
    ModelBackend.getModels(organizationName).then((res) => {
      if (res.status === "error") {
        Setting.showMessage("error", res.msg);
        return;
      }
  ...
  getModel(organizationName, modelName) {
    ...
    ModelBackend.getModel(organizationName, modelName).then((res) => {
      this.setState({
        model: res.data,
      });
    });
    ...
    return (
      <Card
        ...
        title={
          <div>
            {this.state.mode === "add"
              ? i18next.t("permission:New Permission")
              : i18next.t("permission:Edit Permission")}
            &nbsp;&nbsp;&nbsp;&nbsp;
            <Button onClick={() => this.submitPermissionEdit(false)}>
              {i18next.t("general:Save")}
            </Button>
            <Button
              style={{ marginLeft: "20px" }}
              type="primary"
              onClick={() => this.submitPermissionEdit(true)}
            >
      ...
      >
        ...
        <Row style={{ marginTop: "20px" }}>
          <Col style={{ marginTop: "5px" }} span={Setting.isMobile() ? 22 : 2}>
            {Setting.getLabel(
              i18next.t("general:Model"),
              i18next.t("general:Model - Tooltip")
            )}{" "}
            :
          </Col>
          <Col span={22}>
        ...
        <Row style={{ marginTop: "20px" }}>
          <Col style={{ marginTop: "5px" }} span={Setting.isMobile() ? 22 : 2}>
            {Setting.getLabel(
              i18next.t("general:Adapter"),
              i18next.t("general:Adapter - Tooltip")
            )}{" "}
            :
          </Col>
          <Col span={22}>
            <Input
              value={this.state.permission.adapter}
              onChange={(e) => {
        ...
        <Row style={{ marginTop: "20px" }}>
          ...
          <Col span={22}>
            <Select
              ...
              onChange={(value) => {
                this.updatePermissionField("users", value);
              }}
              options={[
        ...
        <Row style={{ marginTop: "20px" }}>
          ...
          <Col span={22}>
            <Select
              ...
              onChange={(value) => {
                this.updatePermissionField("groups", value);
              }}
              options={[
                Setting.getOption(i18next.t("organization:All"), "*"),
        ...
        <Row style={{ marginTop: "20px" }}>
          ...
          <Col span={22}>
            <Select
              ...
              placeholder={
                this.hasRoleDefinition(this.state.model)
                  ? ""
                  : "This field is disabled because the model is empty or it doesn't support RBAC (in another word, doesn't contain [role_definition])"
              }
              virtual={false}
              mode="multiple"
              style={{ width: "100%" }}
              ...
              onChange={(value) => {
                this.updatePermissionField("roles", value);
              }}
              options={[
                Setting.getOption(i18next.t("organization:All"), "*"),
                ...this.state.roles
                  .filter(
                    (roles) =>
                      roles.owner !== this.state.roles.owner ||
                      roles.name !== this.state.roles.name
                  )
                  .map((permission) =>
                    Setting.getOption(
                      `${permission.owner}/${permission.name}`,
                      `${permission.owner}/${permission.name}`
                    )
                  ),
              ]}
        ...
        <Row style={{ marginTop: "20px" }}>
          ...
          <Col span={22}>
            <Select
              ...
              mode="tags"
              style={{ width: "100%" }}
              value={this.state.permission.domains}
              onChange={(value) => {
                this.updatePermissionField("domains", value);
              }}
              options={[
                Setting.getOption(i18next.t("organization:All"), "*"),
                ...this.state.permission.domains
                  .filter((domain) => domain !== "*")
                  .map((domain) => Setting.getOption(domain, domain)),
              ]}
            />
          </Col>
        </Row>
        <Row style={{ marginTop: "20px" }}>
          <Col style={{ marginTop: "5px" }} span={Setting.isMobile() ? 22 : 2}>
            {Setting.getLabel(
              i18next.t("permission:Resource type"),
              i18next.t("permission:Resource type - Tooltip")
            )}{" "}
            :
          </Col>
          <Col span={22}>
            <Select
              ...
              options={[
                {
                  value: "Application",
                  name: i18next.t("general:Application"),
                },
        ...
        <Row style={{ marginTop: "20px" }}>
          <Col style={{ marginTop: "5px" }} span={Setting.isMobile() ? 22 : 2}>
            {Setting.getLabel(
              ...
              i18next.t("permission:Resources - Tooltip")
            )}{" "}
            :
          </Col>
          <Col span={22}>
            <Select
              virtual={false}
              mode={
                this.state.permission.resourceType === "Custom"
                  ? "tags"
                  : "multiple"
              }
              style={{ width: "100%" }}
              ...
              options={[
                ...
                ...this.state.resources.map((resource) =>
                  Setting.getOption(`${resource.name}`, `${resource.name}`)
                ),
              ]}
            />
          </Col>
        ...
        <Row style={{ marginTop: "20px" }}>
          <Col style={{ marginTop: "5px" }} span={Setting.isMobile() ? 22 : 2}>
            {Setting.getLabel(
              i18next.t("permission:Actions"),
              i18next.t("permission:Actions - Tooltip")
            )}{" "}
            :
          </Col>
          <Col span={22}>
        ...
        <Row style={{ marginTop: "20px" }}>
          <Col style={{ marginTop: "5px" }} span={Setting.isMobile() ? 22 : 2}>
            {Setting.getLabel(
              i18next.t("permission:Effect"),
              i18next.t("permission:Effect - Tooltip")
            )}{" "}
            :
          </Col>
          <Col span={22}>
        ...
        <Row style={{ marginTop: "20px" }}>
          <Col style={{ marginTop: "5px" }} span={Setting.isMobile() ? 22 : 2}>
            {Setting.getLabel(
              ...
              i18next.t("permission:Submitter - Tooltip")
            )}{" "}
            :
          </Col>
          <Col span={22}>
        ...
        <Row style={{ marginTop: "20px" }}>
          <Col style={{ marginTop: "5px" }} span={Setting.isMobile() ? 22 : 2}>
            {Setting.getLabel(
              i18next.t("permission:Approver"),
              i18next.t("permission:Approver - Tooltip")
            )}{" "}
            :
          </Col>
          <Col span={22}>
            <Input
              disabled={true}
              value={this.state.permission.approver}
              onChange={(e) => {
        ...
        <Row style={{ marginTop: "20px" }}>
          <Col style={{ marginTop: "5px" }} span={Setting.isMobile() ? 22 : 2}>
            {Setting.getLabel(
              i18next.t("permission:Approve time"),
              i18next.t("permission:Approve time - Tooltip")
            )}{" "}
            :
          </Col>
          <Col span={22}>
            <Input
              disabled={true}
              value={Setting.getFormattedDate(
                this.state.permission.approveTime
              )}
              onChange={(e) => {
        ...
        <Row style={{ marginTop: "20px" }}>
          ...
          <Col span={22}>
            <Select
              ...
              style={{ width: "100%" }}
              value={this.state.permission.state}
              onChange={(value) => {
                if (this.state.permission.state !== value) {
                  if (value === "Approved") {
                    this.updatePermissionField(
                      "approver",
  ...
  submitPermissionEdit(exitAfterSave) {
    if (
      this.state.permission.users.length === 0 &&
      this.state.permission.roles.length === 0
    ) {
      Setting.showMessage(
        "error",
        "The users and roles cannot be empty at the same time"
      );
      return;
    }
    ...
    if (
      ...
      this.state.permission.submitter !== this.props.account.name
    ) {
      Setting.showMessage(
        "error",
        "A normal user can only modify the permission submitted by itself"
      );
      return;
    }

    const permission = Setting.deepCopy(this.state.permission);
    PermissionBackend.updatePermission(
      this.state.organizationName,
      this.state.permissionName,
      permission
    )
        ...
        if (res.status === "ok") {
          ...
          if (exitAfterSave) {
            ...
            this.props.history.push(
              `/permissions/${this.state.permission.owner}/${encodeURIComponent(
                this.state.permission.name
              )}`
            );
          }
        } else {
    ...
    return (
      <div>
        ...
        <div style={{ marginTop: "20px", marginLeft: "40px" }}>
          <Button size="large" onClick={() => this.submitPermissionEdit(false)}>
            {i18next.t("general:Save")}
          </Button>
          <Button
            style={{ marginLeft: "20px" }}
