...
export function getPlans(
  ...
  return fetch(
    `${Setting.ServerUrl}/api/get-plans?owner=${owner}&p=${page}&pageSize=${pageSize}&field=${field}&value=${value}&sortField=${sortField}&sortOrder=${sortOrder}`,
    {
      method: "GET",
      credentials: "include",
      headers: {
...
export function getPlan(owner, name, includeOption = false) {
  return fetch(
    `${Setting.ServerUrl}/api/get-plan?id=${owner}/${encodeURIComponent(
      name
...
export function updatePlan(owner, name, plan) {
  ...
  return fetch(
    `${Setting.ServerUrl}/api/update-plan?id=${owner}/${encodeURIComponent(
      name
    )}`,
    {
      method: "POST",
