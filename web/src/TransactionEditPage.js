...
class TransactionEditPage extends React.Component {
  ...
  getTransaction() {
    TransactionBackend.getTransaction(
  ...
  submitTransactionEdit(exitAfterSave) {
    ...
    TransactionBackend.updateTransaction(
      this.state.transaction.owner,
      this.state.transactionName,
      transaction
    )
      .then((res) => {
        if (res.status === "ok") {
          ...
          if (exitAfterSave) {
            ...
            this.props.history.push(
              `/transactions/${this.state.organizationName}/${this.state.transaction.name}`
            );
          }
        } else {
          Setting.showMessage(
  ...
  getApplication() {
    ApplicationBackend.getApplication("admin", this.state.applicationName).then(
      (res) => {
        ...
        if (
          application.grantTypes === null ||
          application.grantTypes === undefined ||
          application.grantTypes.length === 0
        ) {
    ...
    return (
      <Card
        ...
        title={
          <div>
            {this.state.mode === "add"
              ? i18next.t("transaction:New Transaction")
              : i18next.t("transaction:Edit Transaction")}
            &nbsp;&nbsp;&nbsp;&nbsp;
            <Button onClick={() => this.submitTransactionEdit(false)}>
              {i18next.t("general:Save")}
            </Button>
            <Button
              style={{ marginLeft: "20px" }}
              type="primary"
              onClick={() => this.submitTransactionEdit(true)}
            >
      ...
      >
        <Row style={{ marginTop: "10px" }}>
          ...
          <Col span={22}>
            <Input
              disabled={true}
              value={this.state.transaction.owner}
              onChange={(e) => {
        ...
        <Row style={{ marginTop: "20px" }}>
          ...
          <Col span={22}>
            <Input
              disabled={true}
              value={this.state.transaction.displayName}
              onChange={(e) => {
        ...
        <Row style={{ marginTop: "20px" }}>
          ...
          <Col span={22}>
            <Input
              disabled={true}
              value={this.state.transaction.provider}
              onChange={(e) => {
        ...
        <Row style={{ marginTop: "20px" }}>
          <Col style={{ marginTop: "5px" }} span={Setting.isMobile() ? 22 : 2}>
            {Setting.getLabel(
              ...
              i18next.t("provider:Category - Tooltip")
            )}{" "}
            :
          </Col>
          <Col span={22}>
        ...
        <Row style={{ marginTop: "20px" }}>
          ...
          <Col span={22}>
            <Input
              disabled={true}
              value={this.state.transaction.productName}
              onChange={(e) => {
        ...
        <Row style={{ marginTop: "20px" }}>
          <Col style={{ marginTop: "5px" }} span={Setting.isMobile() ? 22 : 2}>
            {Setting.getLabel(
              i18next.t("user:Tag"),
              i18next.t("transaction:Tag - Tooltip")
            )}{" "}
            :
          </Col>
          <Col span={22}>
        ...
        <Row style={{ marginTop: "20px" }}>
          <Col style={{ marginTop: "5px" }} span={Setting.isMobile() ? 22 : 2}>
            {Setting.getLabel(
              i18next.t("transaction:Amount"),
              i18next.t("transaction:Amount - Tooltip")
            )}{" "}
            :
          </Col>
          <Col span={22}>
        ...
        <Row style={{ marginTop: "20px" }}>
          ...
          <Col span={22}>
            <Input
              disabled={true}
              value={this.state.transaction.application}
              onChange={(e) => {
                // this.updatePaymentField('amount', e.target.value);
              }}
            />
    ...
    return (
      <div>
        ...
        <div style={{ marginTop: "20px", marginLeft: "40px" }}>
          <Button
            size="large"
            onClick={() => this.submitTransactionEdit(false)}
          >
            {i18next.t("general:Save")}
          </Button>
          <Button
