...
class SubscriptionEditPage extends React.Component {
  ...
  getSubscription() {
    SubscriptionBackend.getSubscription(
  ...
  getPricings(organizationName) {
    PricingBackend.getPricings(organizationName).then((res) => {
      this.setState({
        pricings: res.data,
      });
    });
  ...
  getPlans(organizationName) {
    PlanBackend.getPlans(organizationName).then((res) => {
      this.setState({
        plans: res.data,
      });
    });
    ...
    return (
      <Card
        ...
        title={
          <div>
            {this.state.mode === "add"
              ? i18next.t("subscription:New Subscription")
              : i18next.t("subscription:Edit Subscription")}
            &nbsp;&nbsp;&nbsp;&nbsp;
            <Button onClick={() => this.submitSubscriptionEdit(false)}>
              {i18next.t("general:Save")}
            </Button>
            <Button
              style={{ marginLeft: "20px" }}
              type="primary"
              onClick={() => this.submitSubscriptionEdit(true)}
            >
      ...
      >
        <Row style={{ marginTop: "10px" }}>
          ...
          <Col span={22}>
            <Select
              virtual={false}
              style={{ width: "100%" }}
              value={this.state.subscription.owner}
              onChange={(owner) => {
        ...
        <Row style={{ marginTop: "20px" }}>
          <Col style={{ marginTop: "5px" }} span={Setting.isMobile() ? 22 : 2}>
            {Setting.getLabel(
              i18next.t("subscription:Start time"),
              i18next.t("subscription:Start time - Tooltip")
            )}
          </Col>
          <Col span={22}>
            <DatePicker
              value={dayjs(this.state.subscription.startTime)}
              onChange={(value) => {
        ...
        <Row style={{ marginTop: "20px" }}>
          <Col style={{ marginTop: "5px" }} span={Setting.isMobile() ? 22 : 2}>
            {Setting.getLabel(
              i18next.t("subscription:End time"),
              i18next.t("subscription:End time - Tooltip")
            )}
          </Col>
          <Col span={22}>
            <DatePicker
              value={dayjs(this.state.subscription.endTime)}
              onChange={(value) => {
        ...
        <Row style={{ marginTop: "20px" }}>
          <Col style={{ marginTop: "5px" }} span={Setting.isMobile() ? 22 : 2}>
            {Setting.getLabel(
              i18next.t("plan:Period"),
              i18next.t("plan:Period - Tooltip")
            )}{" "}
            :
          </Col>
          <Col span={22}>
            <Select
              defaultValue={
                this.state.subscription.period === ""
                  ? "Monthly"
                  : this.state.subscription.period
              }
              onChange={(value) => {
        ...
        <Row style={{ marginTop: "20px" }}>
          ...
          <Col span={22}>
            <Select
              ...
              onChange={(value) => {
                this.updateSubscriptionField("user", value);
              }}
              options={this.state.users.map((user) =>
                Setting.getOption(user.name, user.name)
              )}
            />
          </Col>
        </Row>
        <Row style={{ marginTop: "20px" }}>
          <Col style={{ marginTop: "5px" }} span={Setting.isMobile() ? 22 : 2}>
            {Setting.getLabel(
              i18next.t("general:Pricing"),
              i18next.t("general:Pricing - Tooltip")
            )}{" "}
            :
          </Col>
          <Col span={22}>
            <Select
              virtual={false}
              style={{ width: "100%" }}
              value={this.state.subscription.pricing}
              onChange={(value) => {
                this.updateSubscriptionField("pricing", value);
              }}
              options={this.state.pricings.map((pricing) =>
                Setting.getOption(pricing.name, pricing.name)
              )}
            />
          </Col>
        </Row>
        <Row style={{ marginTop: "20px" }}>
          <Col style={{ marginTop: "5px" }} span={Setting.isMobile() ? 22 : 2}>
            {Setting.getLabel(
              i18next.t("general:Plan"),
              i18next.t("general:Plan - Tooltip")
            )}{" "}
            :
          </Col>
          <Col span={22}>
            <Select
              ...
              onChange={(value) => {
                this.updateSubscriptionField("plan", value);
              }}
              options={this.state.plans.map((plan) =>
                Setting.getOption(plan.name, plan.name)
              )}
            />
          </Col>
        </Row>
        <Row style={{ marginTop: "20px" }}>
          <Col style={{ marginTop: "5px" }} span={Setting.isMobile() ? 22 : 2}>
            {Setting.getLabel(
              ...
              i18next.t("general:Payment - Tooltip")
            )}{" "}
            :
          </Col>
          <Col span={22}>
            <Input
              value={this.state.subscription.payment}
              disabled={true}
              onChange={(e) => {
  ...
  submitSubscriptionEdit(exitAfterSave) {
    ...
    SubscriptionBackend.updateSubscription(
      this.state.organizationName,
      this.state.subscriptionName,
      subscription
    )
        ...
        if (res.status === "ok") {
          ...
          if (exitAfterSave) {
            ...
            this.props.history.push(
              `/subscriptions/${this.state.subscription.owner}/${this.state.subscription.name}`
            );
          }
        } else {
          Setting.showMessage(
    ...
    return (
      <div>
        ...
        <div style={{ marginTop: "20px", marginLeft: "40px" }}>
          <Button
            size="large"
            onClick={() => this.submitSubscriptionEdit(false)}
          >
            {i18next.t("general:Save")}
          </Button>
          <Button
