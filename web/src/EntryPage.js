...
class EntryPage extends React.Component {
  ...
  render() {
    const onUpdateApplication = (application) => {
      ...
      const themeData =
        application !== null
          ? Setting.getThemeData(application.organizationObj, application)
          : Conf.ThemeDefault;
    ...
    const onUpdatePricing = (pricing) => {
      ...
      ApplicationBackend.getApplication("admin", pricing.application).then(
        (res) => {
          if (res.status === "error") {
    ...
    if (this.state.application?.ipRestriction) {
      return Util.renderMessageLarge(
        this,
        this.state.application.ipRestriction
      );
    }
    ...
    if (this.state.application?.organizationObj?.ipRestriction) {
      return Util.renderMessageLarge(
        this,
        this.state.application.organizationObj.ipRestriction
      );
    }
    ...
    const themeData =
      this.state.application !== null
        ? Setting.getThemeData(
            this.state.application?.organizationObj,
    ...
    return (
      <React.Fragment>
        ...
        <div
          className={`${
            isDarkMode ? "loginBackgroundDark" : "loginBackground"
          }`}
          style={{
            backgroundColor: themeData.colorPrimary,
            backgroundImage: Setting.inIframe()
              ? null
              : Setting.isMobile()
              ? `url(${this.state.application?.formBackgroundUrlMobile})`
              : `url(${this.state.application?.formBackgroundUrl})`,
          }}
        >
          <Spin
            size="large"
          ...
          <Switch>
            ...
            <Route
              exact
              path="/login/oauth/device/:userCode"
              render={(props) => (
                <LoginPage
                  {...this.props}
                  application={this.state.application}
            ...
            <Route
              exact
              path="/select-plan/:owner/:pricingName"
              render={(props) => (
                <PricingPage
                  {...this.props}
            ...
            <Route
              exact
              path="/captcha"
              render={(props) => <CaptchaPage {...props} />}
            />
          </Switch>
        </div>
      </React.Fragment>
