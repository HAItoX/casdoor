...
class OAuthWidget extends React.Component {
  ...
  getAddressOptions(application) {
    ...
    UserBackend.getAddressOptions(addressUrl).then((addressOptions) => {
      this.setState({
  ...
  getAffiliationOptions(application, user) {
    ...
    UserBackend.getAffiliationOptions(affiliationUrl, code).then(
      (affiliationOptions) => {
        this.setState({
    ...
    return (
      <Row key={provider.name} style={{ marginTop: "20px" }}>
        <Col style={{ marginTop: "5px" }} span={this.props.labelSpan}>
          ...
          <span style={{ marginLeft: "5px" }}>{`${provider.type}:`}</span>
        </Col>
        <Col span={24 - this.props.labelSpan}>
          ...
          {linkedValue === "" ? (
            provider.category === "Web3" ? (
              ...
              <a
                ...
                href={
                  user.id !== account.id
                    ? null
                    : Provider.getAuthUrl(application, provider, "link")
                }
              >
                <Button
                  style={{ marginLeft: "20px", width: linkButtonWidth }}
            ...
            <Button
              disabled={
                !providerItem.canUnlink && !Setting.isAdminUser(account)
              }
              style={{ marginLeft: "20px", width: linkButtonWidth }}
              onClick={() => this.unlinkUser(provider.type, linkedValue)}
            >
  ...
  render() {
    return this.renderIdp(
      this.props.user,
      this.props.application,
      this.props.providerItem
    );
  }
}
