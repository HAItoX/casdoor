      ...
      detection.current = setInterval(async () => {
        if (modelsLoaded && videoRef.current && visible) {
          ...
          if (count % 50 === 0) {
            message.warning(
              i18next.t(
                "login:Please ensure sufficient lighting and align your face in the center of the recognition box"
              )
            );
          } else if (count > 300) {
        ...
        >
          <Space direction={"vertical"} style={{ width: "100%" }}>
            ...
            {modelsLoaded ? (
                ...
                onClick={async () => {
                  ...
                  for (const file of files) {
                    ...
                    const faceIds = await faceapi
                      ...
                      .withFaceDescriptors();
                    if (
                      faceIds[0]?.detection.score > 0.9 &&
                      faceIds[0]?.detection.score > maxScore
                    ) {
              ...
              >
                {" "}
                {i18next.t("application:Generate Face ID")}
              </Button>
            ) : null}
          </Space>
          {currentFaceId && currentFaceId.length !== 0 ? (
            <React.Fragment>
              <div>
                {i18next.t("application:Select")}:
                {files[currentFaceIndex]?.name}
              </div>
              <div>
                <img
                  src={files[currentFaceIndex]?.base64}
                  alt="selected"
                  style={{ width: "100%" }}
                />
              </div>
