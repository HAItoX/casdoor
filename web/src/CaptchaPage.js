...
class CaptchaPage extends React.Component {
  ...
  callback(values) {
    Setting.goToLink(
      `${this.state.redirectUri}?code=${values.captchaToken}&type=${values.captchaType}&secret=${values.clientSecret}&applicationId=${values.applicationId}`
    );
  }
  ...
  render() {
    return this.renderCaptchaModal(this.state.application);
  }
}
