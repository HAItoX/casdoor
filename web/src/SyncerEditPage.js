...
class SyncerEditPage extends React.Component {
  ...
  getSyncer() {
    SyncerBackend.getSyncer("admin", this.state.syncerName).then((res) => {
      if (res.data === null) {
  ...
  getCerts(owner) {
    CertBackend.getCerts(owner).then((res) => {
      this.setState({
        certs: res.data || [],
      });
    });
    ...
    return (
      <Card
        ...
        title={
          <div>
            {this.state.mode === "add"
              ? i18next.t("syncer:New Syncer")
              : i18next.t("syncer:Edit Syncer")}
            &nbsp;&nbsp;&nbsp;&nbsp;
            <Button onClick={() => this.submitSyncerEdit(false)}>
              {i18next.t("general:Save")}
            </Button>
            <Button
              style={{ marginLeft: "20px" }}
              type="primary"
              onClick={() => this.submitSyncerEdit(true)}
            >
      ...
      >
        <Row style={{ marginTop: "10px" }}>
          ...
          <Col span={22}>
            <Select
              ...
              onChange={(value) => {
                this.updateSyncerField("organization", value);
              }}
            >
        ...
        <Row style={{ marginTop: "20px" }}>
          <Col style={{ marginTop: "5px" }} span={Setting.isMobile() ? 22 : 2}>
            {Setting.getLabel(
              ...
              i18next.t("provider:Type - Tooltip")
            )}{" "}
            :
          </Col>
          <Col span={22}>
            <Select
              virtual={false}
              style={{ width: "100%" }}
              value={this.state.syncer.type}
              onChange={(value) => {
                this.updateSyncerField("type", value);
                const syncer = this.state.syncer;
                syncer["tableColumns"] = this.getSyncerTableColumns(
                  this.state.syncer
                );
                syncer.table =
                  value === "Keycloak"
                    ? "user_entity"
                    : this.state.syncer.table;
                this.setState({
                  syncer: syncer,
                });
              }}
            >
              {["Database", "Keycloak"].map((item, index) => (
        ...
        <Row style={{ marginTop: "20px" }}>
          <Col style={{ marginTop: "5px" }} span={Setting.isMobile() ? 22 : 2}>
            {Setting.getLabel(
              i18next.t("syncer:Database type"),
              i18next.t("syncer:Database type - Tooltip")
            )}{" "}
            :
          </Col>
          <Col span={22}>
            <Select
              virtual={false}
              style={{ width: "100%" }}
              value={this.state.syncer.databaseType}
              onChange={(value) => {
        ...
        {this.state.syncer.databaseType !== "postgres" ? null : (
          <Row style={{ marginTop: "20px" }}>
            ...
            >
              {Setting.getLabel(
                i18next.t("syncer:SSL mode"),
                i18next.t("syncer:SSL mode - Tooltip")
              )}{" "}
              :
            </Col>
            <Col span={22}>
              <Select
                virtual={false}
                style={{ width: "100%" }}
                value={this.state.syncer.sslMode}
                onChange={(value) => {
                  this.updateSyncerField("sslMode", value);
                }}
              >
                {[
        ...
        <Row style={{ marginTop: "20px" }}>
          ...
          <Col span={22}>
            <Input.Password
              value={this.state.syncer.password}
              onChange={(e) => {
        ...
        <Row style={{ marginTop: "20px" }}>
          ...
          <Col span={22}>
            <Input
              value={this.state.syncer.database}
              onChange={(e) => {
        ...
        {this.state.syncer.databaseType === "mysql" ||
        this.state.syncer.databaseType === "mssql" ||
        this.state.syncer.databaseType === "postgres" ? (
          <Row style={{ marginTop: "20px" }}>
            <Col
              style={{ marginTop: "5px" }}
              span={Setting.isMobile() ? 22 : 2}
            >
              {Setting.getLabel(
                i18next.t("general:SSH type"),
                i18next.t("general:SSH type - Tooltip")
              )}{" "}
              :
            </Col>
            <Col span={22}>
              <Radio.Group
                value={this.state.syncer.sshType}
                buttonStyle="solid"
                onChange={(e) => {
              ...
              >
                ...
                <Radio.Button value="cert">
                  {i18next.t("general:Cert")}
                </Radio.Button>
              </Radio.Group>
            </Col>
          </Row>
        ) : null}
        {(this.state.syncer.sshType &&
          this.state.syncer.databaseType === "mysql") ||
        this.state.syncer.databaseType === "mssql" ||
        this.state.syncer.databaseType === "postgres" ? (
          <React.Fragment>
            <Row style={{ marginTop: "20px" }}>
              ...
              <Col span={22}>
                <Input
                  prefix={<LinkOutlined />}
                  value={this.state.syncer.sshHost}
                  onChange={(e) => {
            ...
            <Row style={{ marginTop: "20px" }}>
              ...
              <Col span={22}>
                <InputNumber
                  value={this.state.syncer.sshPort}
                  onChange={(value) => {
            ...
            <Row style={{ marginTop: "20px" }}>
              ...
              <Col span={22}>
                <Input
                  value={this.state.syncer.sshUser}
                  onChange={(e) => {
            ...
            {this.state.syncer.sshType === "password" &&
            (this.state.syncer.databaseType === "mysql" ||
              this.state.syncer.databaseType === "mssql" ||
              this.state.syncer.databaseType === "postgres") ? (
              <Row style={{ marginTop: "20px" }}>
                ...
                <Col span={22}>
                  <Input.Password
                    value={this.state.syncer.sshPassword}
                    onChange={(e) => {
              ...
              <Row style={{ marginTop: "20px" }}>
                ...
                >
                  {Setting.getLabel(
                    i18next.t("general:SSH cert"),
                    i18next.t("general:Cert - Tooltip")
                  )}{" "}
                  :
                </Col>
                <Col span={22}>
                  <Select
                    virtual={false}
                    style={{ width: "100%" }}
                    value={this.state.syncer.cert}
                    onChange={(value) => {
                      this.updateSyncerField("cert", value);
                    }}
                  >
        ...
        <Row style={{ marginTop: "20px" }}>
          <Col style={{ marginTop: "5px" }} span={Setting.isMobile() ? 22 : 2}>
            {Setting.getLabel(
              i18next.t("syncer:Table"),
              i18next.t("syncer:Table - Tooltip")
            )}{" "}
            :
          </Col>
          <Col span={22}>
            <Input
              value={this.state.syncer.table}
              onChange={(e) => {
        ...
        <Row style={{ marginTop: "20px" }}>
          <Col style={{ marginTop: "5px" }} span={Setting.isMobile() ? 22 : 2}>
            {Setting.getLabel(
              i18next.t("provider:DB test"),
              i18next.t("provider:DB test - Tooltip")
            )}{" "}
            :
          </Col>
          <Col span={2}>
            <Button
              type={"primary"}
              loading={this.state.testDbLoading}
              onClick={() => {
                ...
                SyncerBackend.testSyncerDb(this.state.syncer)
                  .then((res) => {
                    if (res.status === "ok") {
                      ...
                      Setting.showMessage(
                        "success",
                        i18next.t("syncer:Connect successfully")
                      );
                    } else {
                      this.setState({ testDbLoading: false });
                      Setting.showMessage(
                        "error",
                        `${i18next.t("syncer:Failed to connect")}: ${res.msg}`
                      );
                    }
                  })
                  .catch((error) => {
        ...
        <Row style={{ marginTop: "20px" }}>
          <Col style={{ marginTop: "5px" }} span={Setting.isMobile() ? 22 : 2}>
            {Setting.getLabel(
              i18next.t("syncer:Table columns"),
              i18next.t("syncer:Table columns - Tooltip")
            )}{" "}
            :
          </Col>
          <Col span={22}>
            <SyncerTableColumnTable
              ...
              onUpdateTable={(value) => {
                this.updateSyncerField("tableColumns", value);
              }}
            />
          </Col>
        </Row>
        <Row style={{ marginTop: "20px" }}>
          <Col style={{ marginTop: "5px" }} span={Setting.isMobile() ? 22 : 2}>
            {Setting.getLabel(
              i18next.t("syncer:Affiliation table"),
              i18next.t("syncer:Affiliation table - Tooltip")
            )}{" "}
            :
          </Col>
          <Col span={22}>
            <Input
              value={this.state.syncer.affiliationTable}
              onChange={(e) => {
        ...
        <Row style={{ marginTop: "20px" }}>
          <Col style={{ marginTop: "5px" }} span={Setting.isMobile() ? 22 : 2}>
            {Setting.getLabel(
              i18next.t("syncer:Avatar base URL"),
              i18next.t("syncer:Avatar base URL - Tooltip")
            )}{" "}
            :
          </Col>
          <Col span={22}>
            <Input
              prefix={<LinkOutlined />}
              value={this.state.syncer.avatarBaseUrl}
              onChange={(e) => {
        ...
        <Row style={{ marginTop: "20px" }}>
          <Col style={{ marginTop: "5px" }} span={Setting.isMobile() ? 22 : 2}>
            {Setting.getLabel(
              ...
              i18next.t("syncer:Sync interval - Tooltip")
            )}{" "}
            :
          </Col>
          <Col span={22}>
            <InputNumber
              value={this.state.syncer.syncInterval}
              onChange={(value) => {
        ...
        <Row style={{ marginTop: "20px" }}>
          <Col style={{ marginTop: "5px" }} span={Setting.isMobile() ? 22 : 2}>
            {Setting.getLabel(
              i18next.t("syncer:Error text"),
              i18next.t("syncer:Error text - Tooltip")
            )}{" "}
            :
          </Col>
          <Col span={22}>
        ...
        <Row style={{ marginTop: "20px" }}>
          <Col style={{ marginTop: "5px" }} span={Setting.isMobile() ? 19 : 2}>
            {Setting.getLabel(
              i18next.t("syncer:Is read-only"),
              i18next.t("syncer:Is read-only - Tooltip")
            )}{" "}
            :
          </Col>
          <Col span={1}>
            <Switch
              checked={this.state.syncer.isReadOnly}
              onChange={(checked) => {
        ...
        <Row style={{ marginTop: "20px" }}>
          ...
          <Col span={1}>
            <Switch
              checked={this.state.syncer.isEnabled}
              onChange={(checked) => {
  ...
  submitSyncerEdit(exitAfterSave) {
    ...
    SyncerBackend.updateSyncer(
      this.state.syncer.owner,
      this.state.syncerName,
      syncer
    )
      .then((res) => {
        if (res.status === "ok") {
    ...
    return (
      <div>
        ...
        <div style={{ marginTop: "20px", marginLeft: "40px" }}>
          <Button size="large" onClick={() => this.submitSyncerEdit(false)}>
            {i18next.t("general:Save")}
          </Button>
          <Button
            style={{ marginLeft: "20px" }}
