    ...
    return (
      <Card
        ...
        title={
          <div>
            {this.state.mode === "add"
              ? i18next.t("model:New Model")
              : i18next.t("model:Edit Model")}
            &nbsp;&nbsp;&nbsp;&nbsp;
            <Button onClick={() => this.submitModelEdit(false)}>
              {i18next.t("general:Save")}
            </Button>
            <Button
              style={{ marginLeft: "20px" }}
              type="primary"
              onClick={() => this.submitModelEdit(true)}
            >
      ...
      >
          ...
          <Col span={22}>
            <Select
              ...
              disabled={
                ...
                Setting.builtInObject(this.state.model)
              }
              value={this.state.model.owner}
              onChange={(value) => {
                this.updateModelField("owner", value);
              }}
            >
        ...
        <Row style={{ marginTop: "20px" }}>
          ...
          <Col span={22}>
            <Input
              disabled={Setting.builtInObject(this.state.model)}
        ...
        <Row style={{ marginTop: "20px" }}>
          <Col style={{ marginTop: "5px" }} span={Setting.isMobile() ? 22 : 2}>
            {Setting.getLabel(
              i18next.t("model:Model text"),
              i18next.t("model:Model text - Tooltip")
            )}{" "}
            :
          </Col>
          <Col span={22}>
        ...
        if (res.status === "ok") {
          ...
          if (exitAfterSave) {
            ...
            this.props.history.push(
              `/models/${this.state.model.owner}/${this.state.model.name}`
            );
          }
        } else {
          Setting.showMessage(
    ...
    return (
      <div>
        ...
        <div style={{ marginTop: "20px", marginLeft: "40px" }}>
          <Button size="large" onClick={() => this.submitModelEdit(false)}>
            {i18next.t("general:Save")}
          </Button>
          <Button
            style={{ marginLeft: "20px" }}
