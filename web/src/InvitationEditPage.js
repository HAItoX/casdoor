...
class InvitationEditPage extends React.Component {
  ...
  getInvitation() {
    InvitationBackend.getInvitation(
      this.state.organizationName,
      this.state.invitationName
    ).then((res) => {
      if (res.data === null) {
  ...
  copySignupLink() {
    ...
    if (this.state.invitation.owner === "built-in") {
      ...
      const selectedOrganization = Setting.getArrayItem(
        this.state.organizations,
        "name",
        this.state.invitation.owner
      );
      ...
      if (!defaultApplication) {
        Setting.showMessage(
          "error",
          i18next.t(
            "invitation:You need to first specify a default application for organization: "
    ...
    copy(
      `${window.location.origin}/signup/${defaultApplication}?invitationCode=${this.state.invitation?.defaultCode}`
    );
    Setting.showMessage(
      "success",
  ...
  renderInvitation() {
    const isCreatedByPlan =
      this.state.invitation.tag === "auto_created_invitation_for_plan";
    return (
      <Card
        size="small"
        title={
          <div>
            {this.state.mode === "add"
              ? i18next.t("invitation:New Invitation")
              : i18next.t("invitation:Edit Invitation")}
            &nbsp;&nbsp;&nbsp;&nbsp;
            <Button onClick={() => this.submitInvitationEdit(false)}>
              {i18next.t("general:Save")}
            </Button>
            <Button
              style={{ marginLeft: "20px" }}
              type="primary"
              onClick={() => this.submitInvitationEdit(true)}
            >
      ...
      >
        <Row style={{ marginTop: "10px" }}>
          ...
          <Col span={22}>
            <Select
              ...
              style={{ width: "100%" }}
              disabled={
                !Setting.isAdminUser(this.props.account) || isCreatedByPlan
              }
              value={this.state.invitation.owner}
              onChange={(value) => {
        ...
        <Row style={{ marginTop: "20px" }}>
          ...
          <Col span={22}>
            <Input
              value={this.state.invitation.name}
        ...
        <Row style={{ marginTop: "20px" }}>
          ...
          <Col span={22}>
            <Input
              value={this.state.invitation.displayName}
              onChange={(e) => {
        ...
        <Row style={{ marginTop: "20px" }}>
          <Col style={{ marginTop: "5px" }} span={Setting.isMobile() ? 22 : 2}>
            {Setting.getLabel(
              ...
              i18next.t("invitation:Code - Tooltip")
            )}{" "}
            :
          </Col>
          <Col span={22}>
            <Input
              value={this.state.invitation.code}
              onChange={(e) => {
        ...
        <Row style={{ marginTop: "20px" }}>
          <Col style={{ marginTop: "5px" }} span={Setting.isMobile() ? 22 : 2}>
            {Setting.getLabel(
              i18next.t("invitation:Default code"),
              i18next.t("invitation:Default code - Tooltip")
            )}{" "}
            :
          </Col>
          <Col span={22}>
            <Input
              value={this.state.invitation.defaultCode}
              onChange={(e) => {
                this.updateInvitationField("defaultCode", e.target.value);
              }}
            />
        ...
        <Row style={{ marginTop: "20px" }}>
          <Col style={{ marginTop: "5px" }} span={Setting.isMobile() ? 22 : 2}>
            {Setting.getLabel(
              i18next.t("invitation:Quota"),
              i18next.t("invitation:Quota - Tooltip")
            )}{" "}
            :
          </Col>
          <Col span={22}>
            <InputNumber
              min={0}
              value={this.state.invitation.quota}
              onChange={(value) => {
        ...
        <Row style={{ marginTop: "20px" }}>
          <Col style={{ marginTop: "5px" }} span={Setting.isMobile() ? 22 : 2}>
            {Setting.getLabel(
              i18next.t("invitation:Used count"),
              i18next.t("invitation:Used count - Tooltip")
            )}{" "}
            :
          </Col>
          <Col span={22}>
            <InputNumber
              min={0}
              max={this.state.invitation.quota}
              value={this.state.invitation.usedCount}
              onChange={(value) => {
        ...
        <Row style={{ marginTop: "20px" }}>
          ...
          <Col span={22}>
            <Select
              virtual={false}
              style={{ width: "100%" }}
              value={this.state.invitation.application}
              onChange={(value) => {
                this.updateInvitationField("application", value);
              }}
              options={[
                { label: i18next.t("general:All"), value: "All" },
                ...this.state.applications.map((application) =>
                  Setting.getOption(application.name, application.name)
                ),
              ]}
            />
          </Col>
        ...
        <Row style={{ marginTop: "20px" }}>
          <Col style={{ marginTop: "5px" }} span={Setting.isMobile() ? 22 : 2}>
            {Setting.getLabel(
              i18next.t("provider:Signup group"),
              i18next.t("provider:Signup group - Tooltip")
            )}{" "}
            :
          </Col>
          <Col span={22}>
            <Select
              ...
              onChange={(value) => {
                this.updateInvitationField("signupGroup", value);
              }}
            >
              <Option key={""} value={""}>
              ...
              {this.state.groups.map((group, index) => (
                <Option key={index} value={`${group.owner}/${group.name}`}>
                  {group.name}
                </Option>
              ))}
            </Select>
        ...
        <Row style={{ marginTop: "20px" }}>
          <Col style={{ marginTop: "5px" }} span={Setting.isMobile() ? 22 : 2}>
            {Setting.getLabel(
              ...
              i18next.t("signup:Username - Tooltip")
            )}{" "}
            :
          </Col>
          <Col span={22}>
            <Input
              value={this.state.invitation.username}
              onChange={(e) => {
        ...
        <Row style={{ marginTop: "20px" }}>
          <Col style={{ marginTop: "5px" }} span={Setting.isMobile() ? 22 : 2}>
            {Setting.getLabel(
              ...
              i18next.t("general:Phone - Tooltip")
            )}{" "}
            :
          </Col>
          <Col span={22}>
            <Input
              value={this.state.invitation.phone}
              onChange={(e) => {
        ...
        <Row style={{ marginTop: "20px" }}>
          ...
          <Col span={22}>
            <Select
              virtual={false}
              style={{ width: "100%" }}
              value={this.state.invitation.state}
              onChange={(value) => {
              ...
              options={[
                ...
                {
                  value: "Suspended",
                  name: i18next.t("subscription:Suspended"),
                },
  ...
  submitInvitationEdit(exitAfterSave) {
    ...
    InvitationBackend.updateInvitation(
      this.state.organizationName,
      this.state.invitationName,
      invitation
    )
        ...
        if (res.status === "ok") {
          ...
          if (exitAfterSave) {
            ...
            this.props.history.push(
              `/invitations/${this.state.invitation.owner}/${this.state.invitation.name}`
            );
          }
        } else {
          Setting.showMessage(
    ...
    return (
      <div>
        ...
        <div style={{ marginTop: "20px", marginLeft: "40px" }}>
          <Button size="large" onClick={() => this.submitInvitationEdit(false)}>
            {i18next.t("general:Save")}
          </Button>
          <Button
            style={{ marginLeft: "20px" }}
