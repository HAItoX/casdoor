...
class PlanEditPage extends React.Component {
  ...
  getPlan() {
    PlanBackend.getPlan(this.state.organizationName, this.state.planName).then(
      (res) => {
        if (res.data === null) {
  ...
  getPaymentProviders(organizationName) {
    ProviderBackend.getProviders(organizationName).then((res) => {
      if (res.status === "ok") {
        this.setState({
          paymentProviders: res.data.filter(
    ...
    return (
      <Card
        ...
        title={
          <div>
            {this.state.mode === "add"
              ? i18next.t("plan:New Plan")
              : i18next.t("plan:Edit Plan")}
            &nbsp;&nbsp;&nbsp;&nbsp;
            <Button onClick={() => this.submitPlanEdit(false)}>
              {i18next.t("general:Save")}
            </Button>
            <Button
              style={{ marginLeft: "20px" }}
              type="primary"
              onClick={() => this.submitPlanEdit(true)}
            >
      ...
      >
        ...
        <Row style={{ marginTop: "20px" }}>
          ...
          <Col span={22}>
            <Input
              value={this.state.plan.displayName}
              onChange={(e) => {
        ...
        <Row style={{ marginTop: "20px" }}>
          <Col style={{ marginTop: "5px" }} span={Setting.isMobile() ? 22 : 2}>
            {Setting.getLabel(
              i18next.t("general:Role"),
              i18next.t("general:Role - Tooltip")
            )}{" "}
            :
          </Col>
          <Col span={22}>
            <Select
              ...
              onChange={(value) => {
                this.updatePlanField("role", value);
              }}
              options={this.state.roles.map((role) =>
                Setting.getOption(role.name, role.name)
              )}
            />
          </Col>
        </Row>
        ...
        <Row style={{ marginTop: "20px" }}>
          <Col style={{ marginTop: "5px" }} span={Setting.isMobile() ? 22 : 2}>
            {Setting.getLabel(
              i18next.t("plan:Price"),
              i18next.t("plan:Price - Tooltip")
            )}{" "}
            :
          </Col>
          <Col span={22}>
        ...
        <Row style={{ marginTop: "20px" }}>
          ...
          <Col span={22}>
            <Select
              virtual={false}
              style={{ width: "100%" }}
              value={this.state.plan.currency}
              onChange={(value) => {
        ...
        <Row style={{ marginTop: "20px" }}>
          ...
          <Col span={22}>
            <Select
              ...
              mode="multiple"
              style={{ width: "100%" }}
              value={this.state.plan.paymentProviders ?? []}
              onChange={(value) => {
                this.updatePlanField("paymentProviders", value);
              }}
            >
        ...
        <Row style={{ marginTop: "20px" }}>
          ...
          <Col span={1}>
            <Switch
              checked={this.state.plan.isEnabled}
              onChange={(checked) => {
        ...
        if (res.status === "ok") {
          ...
          if (exitAfterSave) {
            ...
            this.props.history.push(
              `/plans/${this.state.plan.owner}/${this.state.plan.name}`
            );
          }
        } else {
          Setting.showMessage(
    ...
    return (
      <div>
        ...
        <div style={{ marginTop: "20px", marginLeft: "40px" }}>
          <Button size="large" onClick={() => this.submitPlanEdit(false)}>
            {i18next.t("general:Save")}
          </Button>
          <Button
            style={{ marginLeft: "20px" }}
