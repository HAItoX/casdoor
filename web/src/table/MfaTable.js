...
class MfaTable extends React.Component {
  ...
  addRow(table) {
    const row = {
      name: Setting.getNewRowNameForTable(table, "Please select a MFA method"),
      rule: "Optional",
    };
    if (table === undefined) {
      table = [];
    ...
    const columns = [
      {
        ...
        render: (text, record, index) => {
          return (
            ...
            >
              {Setting.getDeduplicatedArray(MfaItems, table, "name").map(
                (item, index) => (
            ...
            <Select
              ...
              onChange={(value) => {
                ...
                if (value === MfaRuleRequired && requiredCount >= 1) {
                  Setting.showMessage(
                    "error",
                    "Only 1 MFA methods can be required"
                  );
                  return;
                }
