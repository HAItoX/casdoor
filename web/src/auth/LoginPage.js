...
class LoginPage extends React.Component {
  ...
  getCaptchaRule(application) {
    ...
    if (captchaProviderItems) {
      if (
        ...
        captchaProviderItems.some(
          (providerItem) => providerItem.rule === "Internet-Only"
        )
      ) {
  ...
  getApplicationLogin() {
    ...
    if (this.state.type === "cas") {
      loginParams = Util.getCasLoginParameters(
        "admin",
  ...
  onFinish(values) {
    ...
    if (
      ...
      const application = this.getApplicationObj();
      const noModal = application?.signinItems
        .map(
          (signinItem) =>
          ...
          const loginHandler = (res) => {
            ...
            if (responseType === "login") {
              if (res.data3) {
                ...
                Setting.goToLinkSoft(
                  this,
                  `/forget/${this.state.applicationName}`
                );
              }
              Setting.showMessage(
                "success",
              ...
              if (responseMode === "form_post") {
                const params = {
                  ...
                  id_token: responseTypes.includes("id_token")
                    ? res.data
                    : null,
                  token_type: "bearer",
              ...
              if (res.data2.method === "POST") {
                ...
                Setting.goToLink(
                  ...
                  }SAMLResponse=${encodeURIComponent(
                    SAMLResponse
                  )}&RelayState=${oAuthParams.relayState}`
                );
              }
  ...
  switchLoginOrganization(name) {
    ...
    if (window.location.pathname.startsWith("/login/saml/authorize")) {
      Setting.goToLink(
        `/login/saml/authorize/${name}/${
          application.name
        }-org-${name}?${searchParams.toString()}`
      );
      return;
    }
    ...
    if (window.location.pathname.startsWith("/cas")) {
      Setting.goToLink(
        `/cas/${
          application.name
        }-org-${name}/${name}/login?${searchParams.toString()}`
      );
      return;
    }
      ...
      return (
        <Form.Item key={resultItemKey} className="login-button-box">
          ...
          {application?.signinItems
            ...
            .includes(true)
            ? null
            : this.renderCaptchaModal(application, false)}
        </Form.Item>
      );
      ...
      return (
        <Form.Item>
          <div
            key={resultItemKey}
            style={{ width: "100%" }}
            className="login-organization-select"
          >
            <OrganizationSelect
              style={{ width: "100%" }}
              initValue={application.organization}
              onSelect={(value) => {
      ...
      return (
        <Form
          ...
          initialValues={{
            ...
            autoSignin: !application?.signinItems
              .map(
                (signinItem) =>
                  signinItem.name === "Forgot password?" &&
                  signinItem.rule === "Auto sign in - False"
              )
              ?.includes(true),
  ...
  renderCaptchaModal(application, noModal) {
    ...
    const internetOnlyProviderItems = captchaProviderItems.filter(
      (providerItem) => providerItem.rule === "Internet-Only"
    );
    ...
    if (captchaRule === CaptchaRule.Always && alwaysProviderItems.length > 0) {
      ...
      captchaRule === CaptchaRule.Dynamic &&
      dynamicProviderItems.length > 0
    ) {
      provider = dynamicProviderItems[0].provider;
    } else if (
      captchaRule === CaptchaRule.InternetOnly &&
      internetOnlyProviderItems.length > 0
    ) {
  ...
  signInWithWebAuthn(username, values) {
    ...
    return fetch(
      `${Setting.ServerUrl}/api/webauthn/signin/begin?owner=${
  ...
  renderMethodChoiceBox() {
    ...
    const itemsMap = new Map([
      ...
      [
        ...
        { label: i18next.t("login:WebAuthn"), key: "webAuthn" },
      ],
      [
      ...
      [
        generateItemKey("WeChat", "Tab"),
        { label: i18next.t("login:WeChat"), key: "wechat" },
      ],
      [
        generateItemKey("WeChat", "None"),
        { label: i18next.t("login:WeChat"), key: "wechat" },
      ],
    ]);
      ...
      return (
        <div>
          <Tabs
            ...
            items={items}
            size={"small"}
            activeKey={this.state.loginMethod}
            onChange={(key) => {
  ...
  render() {
    ...
    const wechatSigninMethods = application.signinMethods?.filter(
      (method) => method.name === "WeChat" && method.rule === "Login page"
    );
    ...
    return (
      <React.Fragment>
        ...
        >
          ...
          <div
            className={
              Setting.isDarkTheme(this.props.themeAlgorithm)
                ? "login-panel-dark"
                : "login-panel"
            }
          >
            ...
            <div className="login-form">
              <div>{this.renderLoginPanel(application)}</div>
            </div>
            {wechatSigninMethods?.length > 0 ? (
              ...
              >
                <div>
                  <h3 style={{ textAlign: "center", width: 320 }}>
                    {i18next.t(
