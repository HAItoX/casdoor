...
class TokenEditPage extends React.Component {
  ...
  parseAccessToken(accessToken) {
    try {
      const parsedHeader = JSON.stringify(
        jwtDecode(accessToken, { header: true }),
        null,
        2
      );
        ...
        <Row style={{ marginTop: "20px" }}>
          ...
          <Col span={editorWidth}>
            <Button
              ...
              disabled={this.state.token.accessToken === ""}
              onClick={() => {
          ...
          <Col style={{ marginTop: "5px" }} span={Setting.isMobile() ? 22 : 2}>
            {Setting.getLabel(
              i18next.t("token:Parsed result"),
              i18next.t("token:Parsed result - Tooltip")
            )}{" "}
            :
          </Col>
          <Col span={editorWidth}>
