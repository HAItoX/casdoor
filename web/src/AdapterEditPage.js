    ...
    return (
      <Card
        ...
        title={
          <div>
            {this.state.mode === "add"
              ? i18next.t("adapter:New Adapter")
              : i18next.t("adapter:Edit Adapter")}
            &nbsp;&nbsp;&nbsp;&nbsp;
            <Button onClick={() => this.submitAdapterEdit(false)}>
              {i18next.t("general:Save")}
            </Button>
            <Button
              style={{ marginLeft: "20px" }}
              type="primary"
              onClick={() => this.submitAdapterEdit(true)}
            >
      ...
      >
          ...
          <Col span={22}>
            <Select
              ...
              disabled={
                ...
                Setting.builtInObject(this.state.adapter)
              }
              value={this.state.adapter.owner}
              onChange={(value) => {
        ...
        <Row style={{ marginTop: "20px" }}>
          ...
          <Col span={22}>
            <Input
              disabled={Setting.builtInObject(this.state.adapter)}
        ...
        <Row style={{ marginTop: "20px" }}>
          ...
          <Col span={22}>
            <Input
              value={this.state.adapter.table}
              disabled={Setting.builtInObject(this.state.adapter)}
              onChange={(e) => {
        ...
        <Row style={{ marginTop: "20px" }}>
          <Col style={{ marginTop: "5px" }} span={Setting.isMobile() ? 19 : 2}>
            {Setting.getLabel(
              i18next.t("adapter:Use same DB"),
              i18next.t("adapter:Use same DB - Tooltip")
            )}{" "}
            :
          </Col>
          <Col span={1}>
          ...
          <React.Fragment>
            ...
            <Row style={{ marginTop: "20px" }}>
              ...
              <Col span={22}>
                <Select
                  ...
                  disabled={Setting.builtInObject(this.state.adapter)}
                  style={{ width: "100%" }}
                  value={this.state.adapter.databaseType}
                  onChange={(value) => {
                    this.updateAdapterField("databaseType", value);
                  }}
                >
                  {[
                    { id: "mysql", name: "MySQL" },
                  ...
                  ].map((databaseType, index) => (
                    <Option key={index} value={databaseType.id}>
                      {databaseType.name}
                    </Option>
                  ))}
                </Select>
            ...
            <Row style={{ marginTop: "20px" }}>
              ...
              <Col span={22}>
                <Input
                  disabled={Setting.builtInObject(this.state.adapter)}
                  value={this.state.adapter.database}
                  onChange={(e) => {
        ...
        <Row style={{ marginTop: "20px" }}>
          ...
          <Col span={2}>
            <Button
              disabled={
                this.state.organizationName !== this.state.adapter.owner
              }
              type={"primary"}
              onClick={() => {
                AdapterBackend.getPolicies(
                  "",
                  "",
                  `${this.state.adapter.owner}/${this.state.adapter.name}`
                )
                  .then((res) => {
                    if (res.status === "ok") {
                      Setting.showMessage(
        ...
        if (res.status === "ok") {
          ...
          if (exitAfterSave) {
            ...
            this.props.history.push(
              `/adapters/${this.state.adapter.owner}/${this.state.adapter.name}`
            );
          }
        } else {
          Setting.showMessage(
    ...
    return (
      <div>
        ...
        <div style={{ marginTop: "20px", marginLeft: "40px" }}>
          <Button size="large" onClick={() => this.submitAdapterEdit(false)}>
            {i18next.t("general:Save")}
          </Button>
          <Button
            style={{ marginLeft: "20px" }}
