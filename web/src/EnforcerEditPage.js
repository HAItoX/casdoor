...
class EnforcerEditPage extends React.Component {
  ...
  getEnforcer() {
    EnforcerBackend.getEnforcer(
      this.state.organizationName,
      this.state.enforcerName,
      true
    ).then((res) => {
      if (res.data === null) {
  ...
  getAdapters(organizationName) {
    AdapterBackend.getAdapters(organizationName).then((res) => {
      this.setState({
        adapters: res.data || [],
      });
    });
    ...
    return (
      <Card
        ...
        title={
          <div>
            {this.state.mode === "add"
              ? i18next.t("enforcer:New Enforcer")
              : i18next.t("enforcer:Edit Enforcer")}
            &nbsp;&nbsp;&nbsp;&nbsp;
            <Button onClick={() => this.submitEnforcerEdit(false)}>
              {i18next.t("general:Save")}
            </Button>
            <Button
              style={{ marginLeft: "20px" }}
              type="primary"
              onClick={() => this.submitEnforcerEdit(true)}
            >
      ...
      >
          ...
          <Col span={22}>
            <Select
              ...
              disabled={
                ...
                Setting.builtInObject(this.state.enforcer)
              }
              value={this.state.enforcer.owner}
              onChange={(owner) => {
        ...
        <Row style={{ marginTop: "20px" }}>
          ...
          <Col span={22}>
            <Input
              disabled={Setting.builtInObject(this.state.enforcer)}
        ...
        <Row style={{ marginTop: "20px" }}>
          ...
          <Col span={22}>
            <Input
              value={this.state.enforcer.displayName}
              onChange={(e) => {
        ...
        <Row style={{ marginTop: "20px" }}>
          ...
          <Col span={22}>
            <Input
              value={this.state.enforcer.description}
              onChange={(e) => {
        ...
        <Row style={{ marginTop: "20px" }}>
          ...
          <Col span={22}>
            <Select
              ...
              options={this.state.adapters.map((adapter) =>
                Setting.getOption(
                  `${adapter.owner}/${adapter.name}`,
                  `${adapter.owner}/${adapter.name}`
                )
              )}
            />
        ...
        <Row style={{ marginTop: "20px" }}>
          <Col style={{ marginTop: "5px" }} span={Setting.isMobile() ? 22 : 2}>
            {Setting.getLabel(
              i18next.t("adapter:Policies"),
              i18next.t("adapter:Policies - Tooltip")
            )}{" "}
            :
          </Col>
          <Col span={22}>
            <PolicyTable
              enforcer={this.state.enforcer}
              modelCfg={this.state.enforcer?.modelCfg}
  ...
  submitEnforcerEdit(exitAfterSave) {
    ...
    EnforcerBackend.updateEnforcer(
      this.state.organizationName,
      this.state.enforcerName,
      enforcer
    )
        ...
        if (res.status === "ok") {
          ...
          if (exitAfterSave) {
            ...
            this.props.history.push(
              `/enforcers/${this.state.enforcer.owner}/${this.state.enforcer.name}`
            );
          }
        } else {
          Setting.showMessage(
    ...
    return (
      <div>
        ...
        <div style={{ marginTop: "20px", marginLeft: "40px" }}>
          <Button size="large" onClick={() => this.submitEnforcerEdit(false)}>
            {i18next.t("general:Save")}
          </Button>
          <Button
            style={{ marginLeft: "20px" }}
